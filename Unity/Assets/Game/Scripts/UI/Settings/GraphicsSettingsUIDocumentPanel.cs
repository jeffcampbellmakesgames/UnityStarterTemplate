...
namespace Game
{
	...
	/// </summary>
	public sealed class GraphicsSettingsUIDocumentPanel : UIDocumentPanel
	{
		...
		protected override VisualElement TargetRootVisualElement
		{
			get
			{
				return _targetRootVisualElement ??= _uiDocument.rootVisualElement.Q("GraphicsSettingsGroup");
			}
		}

		private VisualElement _targetRootVisualElement;
		private DropdownField _framePerSecondModeDropdown;
		private DropdownField _resolutionDropdown;
		private DropdownField _fullScreenModesDropdown;
		...
		private void Awake()
		{
			...
			_framePerSecondModeDropdown = TargetRootVisualElement.Q<DropdownField>("TargetFramerateDropdown");
			_resolutionDropdown = TargetRootVisualElement.Q<DropdownField>("ResolutionDropdown");
			_fullScreenModesDropdown = TargetRootVisualElement.Q<DropdownField>("WindowModeDropdown");
			_applyChangesButton = TargetRootVisualElement.Q<Button>("ApplyChangesButton");

			// UI Event subscription
			_framePerSecondModeDropdown.RegisterCallback<ChangeEvent<string>>(OnFramePerSecondModeDropdownChanged);
			_resolutionDropdown.RegisterCallback<ChangeEvent<string>>(OnResolutionDropdownChanged);
			_fullScreenModesDropdown.RegisterCallback<ChangeEvent<string>>(OnFullScreenModeDropdownChanged);

			_applyChangesButton.clicked += OnApplyChangesButtonClicked;
		}

		private void OnDestroy()
		{
			_framePerSecondModeDropdown.UnregisterCallback<ChangeEvent<string>>(OnFramePerSecondModeDropdownChanged);
			_resolutionDropdown.UnregisterCallback<ChangeEvent<string>>(OnResolutionDropdownChanged);
			_fullScreenModesDropdown.UnregisterCallback<ChangeEvent<string>>(OnFullScreenModeDropdownChanged);
		...
		private void SetupUI()
		{
			...
			_framePerSecondModeDropdown.choices =
				_framePerSecondModes.Select(x => x.ToString()).ToList();
			...
			_resolutionDropdown.choices =
				_availableResolutions.Select(x => x.ToString()).ToList();
			...
			_fullScreenModesDropdown.choices =
				_availableFullScreenModes.Select(x => x.ToString()).ToList();
		}
		...
		private void UpdateGraphicsUI()
		{
			...
			_framePerSecondModeDropdown.index = currentFramePerSecondModeIndex;
			...
			_fullScreenModesDropdown.index = fullScreenModeIndex;

			_applyChangesButton.pickingMode = ShouldEnableApplyButton
		...
		/// </summary>
		private void OnFramePerSecondModeDropdownChanged(ChangeEvent<string> evt)
		{
			var newIndex = _framePerSecondModeDropdown.index;
		...
		{
			var newIndex = _resolutionDropdown.index;
		...
		/// </summary>
		private void OnFullScreenModeDropdownChanged(ChangeEvent<string> evt)
		{
			var newIndex = _fullScreenModesDropdown.index;
